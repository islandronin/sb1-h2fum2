...
export async function fetchLinkedInProfile(profileUrl: string): Promise<LinkedInProfile> {
  try {
    ...
    const profile: LinkedInProfile = {
      imageUrl: response.data.profile_pic_url || response.data.profilePicture || '',
      jobTitle: response.data.headline || response.data.current_job_title || '',
      about: response.data.summary || response.data.description || '',
    ...
    // Log the mapped profile for debugging
